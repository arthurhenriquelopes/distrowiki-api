...
async def enrich_sheets_manual_endpoint(
    ...
    try:
        ...
        for d in distros:
            try:
                de = ''
                if getattr(d, 'desktop_environments', None):
                    if len(d.desktop_environments) > 0:
                        first = d.desktop_environments[0]
                        de = getattr(first, 'value', str(first))
                if not de:
                    de = getattr(d, 'desktop', '') or ''
                desktop_map[d.name] = de
            except Exception:
                desktop_map[d.name] = ''

        enriched = await enrich_distros_with_groq(distro_names, fields, desktop_map)
...
async def enrich_sheets_auto_endpoint(
    ...
    """
    🔒 PROTEGIDO: Enriquecimento AUTOMÁTICO: atualiza a planilha diretamente.
    """
    sheets_service = GoogleSheetsService()
    try:
        distros = await sheets_service.fetch_all_distros()
...
# Endpoints públicos permanecem iguais
